...
function transformDoc(fn) {
  return doc => {
    const walk = funk => node => {
      const { content = [], ...rest } = node;
      const transformedNode = funk(rest);
      const walkWithFn = walk(funk);
...
expect.extend({
  toEqualDocument(actual, isExpected) {
    ...
    const expected =
      typeof isExpected === 'function' && actual.type && actual.type.schema
