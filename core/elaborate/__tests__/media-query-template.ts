import { MediaQueryTemplate } from '../src/components/mediaquery/_media-query-template';
import { MediaQueryOptions } from '../src/components/mediaquery/media-query';

const template = new MediaQueryTemplate(MediaQueryOptions);

describe('Media query allowed types', () => {
  it('allowed types', () => {
    MediaQueryTemplate.allowedTypes.forEach(value => {
...
describe('Level 3 Media query types', () => {
  it('css3 media queries', () => {
    ...
    expect(template.render('all and (min-width:500px)')).toBe(
      'all and (min-width: 31.25em)',
    );
    expect(template.render('(min-width:500px)')).toBe('(min-width: 31.25em)');
    expect(template.render('all and (orientation:portrait)')).toBe(
      'all and (orientation:portrait)',
    );
    expect(template.render('screen and (device-aspect-ratio: 16/9)')).toBe(
      'screen and (device-aspect-ratio: 16/9)',
    );
    expect(template.render('screen and (color), projection and (color)')).toBe(
    ...
    expect(template.render('all and (min-color-index: 1)')).toBe(
      'all and (min-color-index: 1)',
    );
    expect(template.render('all and (monochrome)')).toBe('all and (monochrome)');
    expect(template.render('all and (min-monochrome: 1)')).toBe(
      'all and (min-monochrome: 1)',
    );
    expect(template.render('print and (min-resolution: 300dpi)')).toBe(
      'print and (min-resolution: 300dpi)',
    );
    expect(template.render('tv and (scan: progressive)')).toBe(
      'tv and (scan: progressive)',
    );
    expect(template.render('handheld and (grid) and (max-width: 15em)')).toBe(
      'handheld and (grid) and (max-width: 15em)',
    );
    expect(template.render('handheld and (grid) and (device-max-height: 7em)')).toBe(
      'handheld and (grid) and (device-max-height: 7em)',
    );
    expect(template.render('screen and (min-width: 400px) and (max-width: 700px)')).toBe(
      'screen and (min-width: 25em) and (max-width: 43.75em)',
    );
    expect(
      template.render('handheld and (min-width: 20em), screen and (min-width: 20em)'),
    ).toBe('handheld and (min-width: 20em), screen and (min-width: 20em)');
  });

  const negativeTemplate = new MediaQueryTemplate({
    ...MediaQueryOptions,
    ...{ breakpoints: { small: 100, medium: -100 } },
  });
  ...
  it('not allowed css3 media queries', () => {
    expect(() => {
      ...
      new Error(
        'Negative lengths "-100" are not allowed for the "width" or "height" media feature.',
      ),
    );
    ...
    expect(() => {
      ...
      new Error(
        'Negative lengths "-100px" are not allowed for the "width", "height" or "color" media feature.',
      ),
    );
    ...
    expect(() => {
      ...
      new Error('Having no space between "and" and the expression is not allowed.'),
    );
...
describe('Breakpoint (Named Down Range)', () => {
  it('Creates a down range out of a medium breakpoint', () => {
    expect(template.render('medium down')).toBe(
      'only screen and (max-width: 63.9375em)',
    );
  });
...
describe('Breakpoint (HIDPI Default/Up Range)', () => {
  it('Creates a x1 HiDPI up range out of a x1 HiDPI breakpoint', () => {
    expect(template.render('hidpi-1')).toBe(
      'only screen and (-webkit-min-device-pixel-ratio: 1), only screen and (min--moz-device-pixel-ratio: 1), only screen and (min-resolution: 96dpi), only screen and (min-resolution: 1dppx)',
    );
  });

  const expect15 =
    'only screen and (-webkit-min-device-pixel-ratio: 1.5), only screen and (min--moz-device-pixel-ratio: 1.5), only screen and (-o-min-device-pixel-ratio: 3/2), only screen and (min-resolution: 144dpi), only screen and (min-resolution: 1.5dppx)';

  it('Creates a x1.5 up HiDPI range out of a x1.5 HiDPI breakpoint', () => {
  ...
  const expect2 =
    'only screen and (-webkit-min-device-pixel-ratio: 2), only screen and (min--moz-device-pixel-ratio: 2), only screen and (-o-min-device-pixel-ratio: 4/2), only screen and (min-resolution: 192dpi), only screen and (min-resolution: 2dppx)';

  it('Creates a x2 HiDPI up range out of a x2 HiDPI breakpoint', () => {
    expect(template.render('hidpi-2')).toBe(expect2);
  });

  it('Creates a x2 up HiDPI up range out of a x2 HiDPI breakpoint', () => {
...
describe('Breakpoint (HIDPI down Range)', () => {
  ...
  it('Creates a x2 HiDPI range ', () => {
    expect(template.render('(resolution: 2dppx)')).toBe('(resolution: 2dppx)');
    expect(template.render('(min-resolution: 2dpi)')).toBe('(min-resolution: 2dpi)');
    expect(template.render('(max-resolution: 2dppx)')).toBe('(max-resolution: 2dppx)');
  });
});
