...
export default class MediaQueryTemplate {
  ...
  render = (value: string): string  => {
    ...
    const level3Regex = new RegExp(
      `(not |only )?(${MediaQueryTemplate.allowedTypes.join('|')})|\\(((?:device-)?(?:max-|min-)?)?(width|height|orientation|(?:device-)?aspect-ratio|color(?:-index)?|monochrome|resolution|scan|grid):? ?(-?\\d+\\/?\\w+?|\\w+)?\\)`,
      's'
    );
    ...
    const hiDPIRegex = new RegExp(`(${this.joinObjectKeys(this.options.hidpiBreakpoints)}) ?(up|down|only)?`, 'ys');
    ...
    mediaQueries.forEach((queryRule, index) => {
      if (queryRule.split('and(').length > 1) {
        throw new Error('Having no space between "and" and the expression is not allowed.');
      }

      const andSplit = queryRule.split(' and ');
      const delimiter = mediaQueries.length >= 2 && mediaQueries.length !== index + 1 ? ',' : '';
      ...
      andSplit.forEach((expression, expressionIndex) => {
        ...
        if (breakpointRegexMatches !== null && (breakpointRegexMatches[1] !== undefined || breakpointRegexMatches[5] !== undefined)) {
          ...
          if (level3RegexMatches[5] !== undefined) {
            ...
            if (level3RegexMatches[4] !== undefined && level3RegexMatches[4] === 'resolution') {
              // @see https://developer.mozilla.org/de/docs/Web/CSS/resolution
              if (type !== undefined && ! ['dpi', 'dpcm', 'dppx'].includes(type)) {
                throw new Error('The resolution value must be followed by a unit identifier ("dpi", "dpcm" or "dppx").');
              }
            } else if (unit < 0) {
              throw new Error(`Negative lengths "${unit}${type || ''}" are not allowed for the "width", "height" or "color" media feature.`);
            }
        ...
        query += andSplit.length >= 2 && andSplit.length !== expressionIndex + 1 ? ' and ' : '';
      });
  ...
  private calculatedSizes = (
    ...
    if (direction === 'only' || direction === 'down') {
      if (name === null) {
        ...
        bpMax = isHidpi ? (bpNext - 1 / stdWebDpi) : (bpNext - 1 <= 0 ? '0' : `${+stripUnit(em(bpNext)) - 1 / 16}em`);
      }
    }

    if (+breakpoint < 0) {
      throw new Error(`Negative lengths "${breakpoint}" are not allowed for the "width" or "height" media feature.`);
    }
  ...
  private generateDpiMediaQuery = (matches: string[], options: MediaQueryOptionsProps): string => {
    ...
    const bpMinDpi =
      bpMinSize !== null ? `${Math.round(+bpMinSize * options.stdWebDpi)}dpi` : bpMinSize;
    const bpMaxDpi =
      bpMaxSize !== null
  ...
  private joinObjectKeys = (object : object): string => {
      return Object.keys(object).sort((a: string, b: string): number => {
        return b.localeCompare(a);
      }).join('|').replace(new RegExp('_', 'g'), '\\_').replace(new RegExp('-', 'g'), '\\-');
  };
};
